    ...
    <div className="flex-1 overflow-auto bg-[#1e1e1e] relative">
      <table className="w-full text-xs border-collapse text-left">
        <thead className="bg-[#252526] sticky top-0 z-10 shadow-sm border-b border-black">
          <tr>
            <th className="p-2 text-center font-mono text-[10px] text-gray-500 w-10 border-r border-black/50 bg-[#252526]">#</th>
            {safeColumns.map((col) => (
              <th
                key={col}
        ...
        <tbody>
          {safeData.map((row, i) => (
            <tr key={i} className="group hover:bg-[#2a2d2e] border-b border-black/20">
              <td className="p-1 border-r border-black/20 font-mono text-[10px] text-gray-600 text-center bg-[#252526]">
                {i + 1}
              </td>
              {safeColumns.map((col) => (
                <td
